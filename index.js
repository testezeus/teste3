	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				only: {
					...
					Badmin: '[ ! ] Perintah ini hanya bisa di gunakan ketika bot menjadi admin! ',
                                        daftarB: `[ BELUM REGISTER ]\nHalo gan !\nKamu belum Register nih, register dulu gan.... \n\nCommand : ${prefix}register nama|umur\nContoh : ${prefix}register Anker|17`,
				}
			}
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        // Options
                        const botName = 'ZEUS BOT'//Nama Bot
				...
				case 'demote':
					...
					if (mentioned.length > 1) {
						...
						for (let _ of mentioned) {
							teks += `Perintah Di Terima Untuk Menurunkan Jabatan Admin :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					} else {
						mentions(`Perintah Di Terima Untuk Menurunkan Jabatan Admin @${mentioned[0].split('@')[0]}\n\nGw Mau Blg Mampus:v*${groupMetadata.subject}*_`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
                                case 'randomhentai':
    ...
    client.sendMessage(from, options, text, { quoted: mek } )
			...
			case 'quotes':
              ...
              case 'owner':
               ...
               client.sendMessage(from, 'Nih nomor ownerku kak, save ya kak nanti di save balik, Dia Gans:v',MessageType.text, { quoted: mek} )
                break
	case 'hidetag':
                client.updatePresence(from, Presence.composing) 
                if (!isRegister) return reply(mess.only.daftarB)
		...
		case 'infonomor':
               client.updatePresence(from, Presence.composing) 
				...
				if (!isUser) return reply(mess.only.userB)
					...
					client.sendMessage(from, nullis, image, {caption: 'Nih kak', quoted: mek})
					break
	case 'kbbi':
            client.updatePresence(from, Presence.composing) 
		...
		case 'glass':                                        
               ...
               if (data.error) return reply(data.error)
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `*[ ZEUS BROADCAST ]*\n\n${body.slice(4)}`})
						}
						reply('')
					} else {
						for (let _ of anu) {
			...
			case 'igstalk':
                     ...
                     hasilsi = `Fullname : ${hmm3.data.fullname}\npengikut : ${hmm3.data.follower}\nMengikuti : ${hmm3.data.following}\nPrivate : ${hmm3.data.private}\nVerified : ${hmm3.data.verified}\nbio : ${hmm3.data.bio}`
			       ...
			       reply(anu.result.jadwalTV)
					break*/
                                ...
                                case 'coffee':
                                        ...
                                        client.sendMessage(from, ajggg4, image, {quoted: mek, caption: 'Nih gambarnya kak...'})
                                        await limitAdd(sender)
                                        break
                                case 'lovepaper':
                                        ajg3 = body.slice(11)
                                ...
                                case 'randomkpop':
                                        ...
                                        client.sendMessage(from, kpp1, image, {quoted: mek, caption: 'Nih kpopnya...'})
                                        await limitAdd(sender)
                                        break
                                case 'fml2':
